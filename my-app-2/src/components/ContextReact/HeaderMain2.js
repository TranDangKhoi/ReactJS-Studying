...
import { useAuth2 } from '../../contexts/auth-context2'
import { useGallery2 } from '../../contexts/gallery-context2'

const HeaderMain2 = () => {
  const { user, setUser } = useAuth2()
  const { cartItems, photos } = useGallery2()
  const favoriteCount = photos.length ? photos.filter((item) => item.isLiked).length : 0
  const cartItemsCount = cartItems.length
  return (
    <div className='flex items-center justify-between h-[90px] shadow-md p-4'>
      {user ? (
        <div className='flex items-center justify-between gap-3'>
          ...
          <span className='text-lg text-black'>Welcome, {user.username}</span>
        </div>
      ) : (
        <span className='text-lg text-black'>Please, login</span>
      )}
      <div>
        <div className='flex items-center justify-between gap-3'>
          <span className='relative w-8'>
            ...
            <span className='absolute top-0 right-0 flex items-center justify-center w-5 h-5 text-sm text-white translate-x-1/2 bg-pink-500 rounded-full '>
              {favoriteCount}
            </span>
          </span>
          <span className='relative w-8'>
      ...
      {user ? (
        <button
          className='p-4 text-lg text-white bg-green-400 rounded-md'
